...
class Trie:
    ...
    def insert(self,  element):
        current = self.root
        for cur_pos in range(Trie.int_size-1, 0, -1):
            cur_bit = 1 if (element & 1 << cur_pos) else 0
            if current.children[cur_bit] is None:
                current.children[cur_bit] = Trie.TrieNode()
...
for tc in range(n_tc):
    arr_length = int(next(input_data))
    arr = list()
    for i in range(arr_length):
        arr.append(int(next(input_data)))
    op.append(problem_solver(arr))
for l in op:
    print(l)
